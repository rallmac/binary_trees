...
/**
 * check_sub_tree_Left - This Function checks if all nodes are
 * smaller than the specified root
 * @node: node in the tree to verify condition
 * @max: value to compare
 * Return: 1 if all nodes are smaller or equal, else 0.
 */
...
/**
 * check_sub_tree_Right - This function checks if all
 * the nodes are bigger than the specified node
 * @node: The node in the tree to verify condition
 * @min: The value to compare
 * Return: 1 if all is bigger or equal, else 0.
 */
...
/**
 * binary_tree_is_bst - This Function says if a tree is a bst or not
 * the process here is first verify that the left node is smaller
